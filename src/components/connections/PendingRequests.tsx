...
export const PendingRequests = () => {
  ...
  const { data: requesterProfiles = [] } = useQuery({
    queryKey: [
      "requesterProfiles",
      connectionRequests.map((req) => req.requesterId),
    ],
    queryFn: async () => {
      const profiles = [];
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600">
          Failed to load pending connection requests.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      {requesterProfiles.map((profile: any) => {
        const request = connectionRequests.find(
          (req) => req.requesterId === profile.id
        );
        return (
          <div
            key={request?.id}
                  ...
                  if (parent) {
                    parent.innerHTML = `
                                        <div class="w-12 h-12 bg-teal-500 flex items-center justify-center rounded-full border-2 border-gray-200">
                                            <span class="text-white font-bold text-lg">${profile.displayName.charAt(
                                              0
                                            )}</span>
                                        </div>
                                    `;
