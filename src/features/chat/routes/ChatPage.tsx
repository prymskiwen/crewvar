...
import { useChatRooms, useChatRoomByUserId, IChatRoom } from '../api/chatApi';
...
export const ChatPage: React.FC = () => {
  ...
  const { data: chatRoomsData, isLoading } = useChatRooms();
  const { data: specificRoomData, isLoading: isLoadingSpecificRoom } = useChatRoomByUserId(userId || '');
  ...
  useEffect(() => {
    if (userId && specificRoomData?.room) {
      setSelectedRoom(specificRoomData.room);
    } else if (!userId) {
      setSelectedRoom(null);
    }
  }, [userId, specificRoomData]);
  ...
  const filteredRooms = useMemo(() => {
    ...
    switch (sortBy) {
      case 'name':
        ...
        break;
      case 'unread':
        filtered = [...filtered].sort((a, b) => b.unread_count - a.unread_count);
        break;
      case 'recent':
      default:
  ...
  if (isLoading || (userId && isLoadingSpecificRoom)) {
    return (
              ...
              <div className="space-y-2 sm:space-y-3">
                {filteredRooms.map((room) => (
                  <div
                    ...
                    className={`flex items-center p-3 sm:p-4 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors ${
                      room.unread_count > 0 ? 'bg-blue-50 border-l-4 border-blue-400' : ''
                    }`}
                  >
