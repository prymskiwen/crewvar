...
export const ConnectionPendingCard: React.FC<
...
> = () => {
  ...
  const handleAccept = async (requestId: string) => {
    ...
    try {
      ...
      toast.success(
        `🎉 Connection accepted! You're now connected with ${userName}`,
        {
          position: "top-right",
          autoClose: 4000,
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-[#069B93] flex items-center">
            <svg
                ...
                <div className="flex-shrink-0 relative">
                  ...
                  <div
                    className={`absolute -bottom-1 -right-1 w-4 h-4  rounded-full border-2 border-white ${
                      request.isOnline ? "bg-green-500" : "bg-red-500"
                    }`}
                  ></div>
                </div>
