...
export const WhosInPortCard = () => {
  ...
  const crewQueryEnabled =
    !!userProfile?.currentShipId && activePortLinks.length > 0;
  ...
  const getConnectionStatus = (memberId: string) => {
    ...
    const isConnected = userConnections.some(
      (connection) => connection.connectedUserId === memberId
    );
  ...
  console.log("WhosInPortCard Debug:", {
    ...
    hasActivePortLinks,
    shouldShowLoading:
      (portLinksLoading || isActuallyLoadingCrew) && userProfile?.currentShipId,
  });
  ...
  if (
    (portLinksLoading || isActuallyLoadingCrew) &&
    userProfile?.currentShipId
  ) {
    return (
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-[#069B93]">
            Crewvar automatically detects when ships are docked together based
            on crew connections, have you checked who is with you today?
          </h3>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
        ...
        <div className="text-center py-6">
          ...
          <p className="text-gray-600 text-sm mb-4 max-w-sm mx-auto">
            <Link
              to="/profile"
              className="text-[#069B93] hover:text-[#058a7a] font-semibold underline"
            >
              Complete your profile
            </Link>{" "}
            to tell us which ship you are on to unlock finding your friends on
            your ship or on another ship.
          </p>
        </div>
      </div>
    );
          ...
          {crew.length > 0 ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-gray-600 text-sm mb-4">
                Finding crew members from the linked ship
              </p>
            </div>
          ) : (
            <div className="text-center py-6">
